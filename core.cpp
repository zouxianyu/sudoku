...
board_t generate_game_board_impl(const board_t &final_board, int mode,
                                 std::pair<int, int> range, bool unique) {
    // 根据难度进一步确认挖空范围
    int req_num = 0;
...
generate_game_boards(int count, int mode, std::pair<int, int> range,
    ...
    for (int i = 0; i < count; i++) {
        result.emplace_back(
                generate_game_board_impl(final_boards[i], mode, range, unique));
    }
    return result;
}
